...
var square_stipples = [
  {x:10, y:10, r:1}, {x:20, y:10, r:2}, {x:30, y:10, r:3}, {x:40, y:10, r:4}, {x:50, y:10, r:1},
  {x:10, y:20, r:2},                                                          {x:50, y:20, r:2},
  {x:10, y:30, r:3},                                                          {x:50, y:30, r:3},
  {x:10, y:40, r:4},                                                          {x:50, y:40, r:4},
  {x:10, y:50, r:1}, {x:20, y:50, r:3}, {x:30, y:50, r:4}, {x:40, y:50, r:1}, {x:50, y:50, r:1},
];

var circle_stipples = [
                     {x:20, y:10, r:1}, {x:30, y:10, r:2}, {x:40, y:10, r:3},
  {x:10, y:20, r:2}, {x:20, y:20, r:2},                    {x:40, y:20, r:4}, {x:50, y:20, r:2},
  {x:10, y:30, r:3},                                                          {x:50, y:30, r:3},
  {x:10, y:40, r:4}, {x:20, y:40, r:3},                    {x:40, y:40, r:1}, {x:50, y:40, r:4},
                     {x:20, y:50, r:4}, {x:30, y:50, r:3}, {x:40, y:50, r:2},
];
...
var shapes = [ square_stipples, circle_stipples, cross_stipples ];
var default_shape = shapes[0];
var numStipples = default_shape.length;

// function particlesToSvgCircles()
var i;
for (i=0; i < numStipples; i++) {
  var element = $( makeSvgElementNS( 'circle' ) )
        .attr( 'cx', default_shape[i].x )
        .attr( 'cy', default_shape[i].y )
        .attr( 'r', default_shape[i].r )
        .attr( 'fill', 'blue' );
...
var t1 = new TimelineMax( { paused: true } );
//var element = domElementsObjsArray[0];
$.each( domElementsObjsArray, function( idx, element ) {
  t1.insert(
    TweenMax.to( element, durationSecs,
      { attr: { cx: shapes[1][idx].x, cy: shapes[1][idx].y, r: shapes[1][idx].r } }
    )
  );
  t1.insert(
    TweenMax.to( element, durationSecs,
      { attr: { cx: shapes[2][idx].x, cy: shapes[2][idx].y, r: shapes[2][idx].r } },
      '+=0.7'
    )
  );
});
...
var numStipples = 16;
$elementsContainerElem.append(
  new Array(numStipples).join( '<circle cx="0" cy="0" r="4" fill="blue"></circle>' ) );
domElementsObjsArray = $elementsContainerElem.children();
//{ attr: { cx: results.nextProfile.particle.props.x,
//          cy: results.nextProfile.particle.props.y,
//},
var t1 = new TimelineMax( { paused: true } );
var element = domElementsObjsArray[0];
var points = [ {x:10, y:10}, {x:200, y:0}, {x:300, y:100}, {x:600, y:50} ];
var bezTween = TweenMax.to(
  element,
  durationSecs,
  { bezier:{ curviness: 1.25, values: points },
